...
class RBM(object):
    ...
    def get_cost_updates(self, lr=0.1, persistent=None, k=1):
        ...
        cost = self.T.mean(self.free_energy(self.input)) - self.T.mean(
            self.free_energy(chain_end))
        # We must not compute the gradient through the gibbs sampling
        gparams = self.T.grad(cost, self.params, consider_constant=[chain_end])

        # constructs the update dictionary
        for gparam, param in zip(gparams, self.params):
            # make sure that the learning rate is of the right dtype
            updates[param] = param - gparam * self.T.cast(
                lr, dtype=self.theano.config.floatX)
        if persistent:
    ...
    def get_pseudo_likelihood_cost(self, updates):
        ...
        bit_i_idx = self.theano.shared(value=0, name='bit_i_idx')

        # binarize the input image by rounding to nearest integer
        xi = self.T.round(self.input)
        ...
        xi_flip = self.T.set_subtensor(xi[:, bit_i_idx], 1 - xi[:, bit_i_idx])

        # calculate free energy with bit flipped
        fe_xi_flip = self.free_energy(xi_flip)

        # equivalent to e^(-FE(x_i)) / (e^(-FE(x_i)) + e^(-FE(x_{\i})))
        cost = self.T.mean(self.n_visible * self.T.log(self.T.nnet.sigmoid(fe_xi_flip -
                                                            fe_xi)))
    ...
    def get_reconstruction_cost(self, updates, pre_sigmoid_nv):
        ...
        cross_entropy = self.T.mean(
            self.T.sum(self.input * self.T.log(self.T.nnet.sigmoid(pre_sigmoid_nv)) +
                  (1 - self.input) * self.T.log(1 - self.T.nnet.sigmoid(pre_sigmoid_nv)),
                  axis=1))
